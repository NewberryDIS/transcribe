...
const PagePage = ( {search} ) => {
    const [ loading, setLoading ] = useState(true)
    const [ pageId, setPageId ] = useState(true)
    const [ itemId, setItemId ] = useState(true)
    useEffect(() => {
        setLoading(false)
        setPageId(search.pageid )
        setItemId(search.itemid )
    }, [])
    return (
        <Wrapper >
            <Global styles={css`
            ...
            <div className="iframeContainer">
                {loading? '':
                <iframe src={`https://publications.newberry.org/transcription/mms-transcribe/scripto/transcribe/${itemId}/${pageId}#transcription`} placeholder={"Loading..."} title="transcription page"  />
            }
            </div>
            <Footer />
        </Wrapper>
...
export default withLocation(PagePage)
