...
/* $statuses = $scripto->getAllPagesTranscriptionStatus($this->doc->getId());

foreach ($statuses as $k=>$s) {
  if ($s == 'Needs Review') {
    $this->doc->setPageTranscriptionStatusComplete($k);
  }
} */
...
jQuery(document).ready(function() {
    ...
    jQuery('#scripto-transcription-page-edit-complete').click(function() {
      jQuery.post(
            <?php echo js_escape(url('scripto/index/page-action')); ?>,
            {
                page_action: 'change-status',
                page: 'transcription',
                status: 'Complete',
    ...
    jQuery('#scripto-transcription-page-edit-needs-review').click(function() {
      jQuery.post(
            <?php echo js_escape(url('scripto/index/page-action')); ?>,
            {
                page_action: 'change-status',
    ...
    jQuery('#scripto-talk-page-edit').click(function() {
        jQuery('#scripto-talk-page-edit').
            prop('disabled', true).
            text('<?php echo __('Saving translation...'); ?>');
        jQuery.post(
            <?php echo js_escape(url('scripto/index/page-action')); ?>,
            {
                page_action: 'edit',
            ...
            function(data) {
                jQuery('#scripto-talk-page-edit').
                    prop('disabled', false).
                    text('<?php echo __('Save translation'); ?>');
    ...
    if (window.location.hash == '#discussion') {
        ...
        jQuery('#scripto-page-show').text('<?php echo __('show translation'); ?>');
    }
	...
	// TEST 
	jQuery('#scripto-transcription-edit-show2').click(function(event) {
        event.preventDefault();
        var clicks = jQuery(this).data('clicks');
        if (!clicks) {
            jQuery(this).text('<?php echo __('hide edit'); ?>');
            jQuery('#scripto-transcription-edit2').slideDown('fast');
        } else {
            jQuery(this).text('<?php echo __('edit'); ?>');
            jQuery('#scripto-transcription-edit2').slideUp('fast');
        }
        jQuery(this).data("clicks", !clicks);
    });
...
<div id="transcribe-title"><h1><?php echo $head['title']; ?></h1></div>
<?php endif; ?>
<div id="primary">
 <?php

            $files = get_db()->getTable('File')->findByItem($page_id, null, "filename");
			$bgimg = "http://publications.newberry.org/transcribe/mms-transcribe/files/original/" . $files[5]["filename"];?>
<?php echo flash(); ?>
    ...
    <div id="scripto-transcribe" class="scripto">
        ...
        <ul class="nav nav-tabs">
            <li class="active"><a href="#"><?php if ($this->doc->getTitle()): ?><?php echo $this->doc->getTitle(); ?><?php else: ?><?php echo __('Untitled Document'); ?><?php endif; ?></a></li>
        <?php if ($this->scripto->isLoggedIn()): ?>
            ...
            <li><a href="<?php echo html_escape(url('scripto/watchlist')); ?>"><?php echo __('Your transcriptions'); ?></a> </li>
        <?php else: ?>
            <li><a href="<?php echo html_escape(url('scripto/index/login')); ?>"><?php echo __('Log in (optional)'); ?></a></li>
            <li><a href="https://publications.newberry.org/mediawiki2017/index.php?title=Special:CreateAccount" target="_blank"><?php echo __('Create an account'); ?></a></li>
        <?php endif; ?>
            <li><a href="<?php echo html_escape(url('scripto/recent-changes')); ?>"><?php echo __('Recent changes'); ?></a></li>
            <li><a href="<?php echo html_escape(url(array('controller' => 'items', 'action' => 'show', 'id' => $this->doc->getId()), 'id')); ?>"><?php echo __('View all pages in this set'); ?></a></li>
            <li><a href="<?php echo html_escape(url(array('controller' => 'files', 'action' => 'show', 'id' => $this->doc->getPageId()), 'id')); ?>"><?php echo __('View file information'); ?></a></li>
        </ul>

        <h2 id="hidetitle"><?php if ($this->doc->getTitle()): ?><?php echo $this->doc->getTitle(); ?><?php else: ?><?php echo __('Untitled Document'); ?><?php endif; ?></h2>
        ...
        <div>
            ...
            <div style="display:none;">image <?php echo html_escape($this->paginationUrls['current_page_number']); ?> of <?php echo html_escape($this->paginationUrls['number_of_pages']); ?></div>
            <div>
		...
		<div class="tab"><button class="tablinks" onclick="openText(event, 'scripto-transcription-edit')">Transcribe</button><!--<button class="tablinks" onclick="openText(event, 'scripto-talk-edit')">Translate</button>--></div>
        <!-- transcription -->
        <div id="scripto-transcription">
        <?php if ($this->doc->canEditTranscriptionPage()): ?>
			
            <div id="scripto-transcription-edit" class="tabcontent">
            ...
            <?php if ($this->doc->isProtectedTranscriptionPage()): ?>
                ...
                <div>
                  ...
                  <?php // echo $this->formButton('scripto-transcription-page-edit-complete', __('This transcription is complete'), array('style' => 'display:block; float:none; margin-bottom:10px;')); ?>
                    <?php // echo $this->formButton('scripto-transcription-page-edit-needs-review', __('This transcription still needs work'), array('style' => 'display:block; float:none;')); ?>
                </div>
                <p><a href="http://publications.newberry.org/digital/mms-transcribe/guidelines" target="_blank"><?php echo __('View guidelines'); ?></a></p>
				<!-- pagination -->
        <p>
        ...
        <!-- discussion changed to translation by MK -->
        <div id="scripto-talk">
            <?php if ($this->doc->canEditTalkPage()): ?>
            <div id="scripto-talk-edit" class="tabcontent" style="display:none;">
                <div><?php echo $this->formTextarea('scripto-talk-page-wikitext', $this->doc->getTalkPageWikitext(), array('cols' => '76', 'rows' => '16')); ?></div>
                <div>
                    <?php echo $this->formButton('scripto-talk-page-edit', __('Save translation'), array('style' => 'display:inline; float:none;')); ?>
                </div>
                <p><a href="http://publications.newberry.org/digital/mms-transcribe/guidelines" target="_blank"><?php echo __('View guidelines'); ?></a></p>
            ...
            <?php endif; ?>
            <h2><?php echo __('Current Page Translation'); ?>
...
<div id="end" data-iframe-height></div>
