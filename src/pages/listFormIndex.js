...
import { ItemProgress } from '../components/progress'
...
function ListFormIndex (){
  ...
  // sessionStorage.setItem('asdf', JSON.stringify({'the': 'whole'}))
// console.log(sessionStorage.getItem('asdf'))
if (sessionStorage.getItem('data') !== null  ){
  const unFilteredItems = data.map((i, index) => {
    let item = {
      id: i.id,
      count: i.files.count,
      date: [],
  ...
  sessionStorage.setItem('data', JSON.stringify(unFilteredItems))
}
let filteredData = JSON.parse(sessionStorage.getItem('data')).filter(i => secretItems.indexOf(i.id) === -1).filter(i => filterFunctions(filters, i)).sort((a,b) => {
  // let aorder = a.dailyPercent === 0 ? a.pc : a.dailyPercent
  // let border = b.dailyPercent === 0 ? b.pc : b.dailyPercent
  ...
  const itemBoxes = filteredData.map((i, index) => <Boc boxProps={i} key={index} />).sort((a,b) => {
    console.log(a.order)
  ...
  return (
      <BocCss>
        <div className="grid-container top">
          ...
          <div className="grid-item">dailyPercent</div>
          <div className="grid-item">pc</div>
          <div className="grid-item">order</div>
          <div className="grid-item">ItemProgress</div>
        </div>
        {itemBoxes}
      </BocCss>
  )
}
const BocCss = styled.div`
...
const Boc = ({boxProps}) => {
  ...
  boxProps.order = order
  return (
    // <div className="grid-container">
    <div className={order - boxProps.pc - 100 === -100 ? "hide grid-container" : "grid-container"}> 
      <div className="grid-item">{boxProps.id} {order} - {boxProps.pc} = {order - boxProps.pc}</div>
      <div className="grid-item">{boxProps.dailyPercent}</div>
      <div className="grid-item">{boxProps.pc}</div>
      <div className="grid-item">{boxProps.order}</div>
      <div className="grid-item"><ItemProgress setOrder={setOrder} pageCount={boxProps.count} itemid={boxProps.id} /></div>
    </div>
  )
}
