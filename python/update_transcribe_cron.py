...
with open('itemsBothPages.json') as json_file:
    ...
    for i in items:
        ...
        filesurl = 'https://transcribe.newberry.org/api/files?item=' + id
        filesfilename = 'data/dataFiles/files' + id + '.json'
        itemurl = 'https://transcribe.newberry.org/api/items/' + id
        itemfilename = 'data/dataFiles/item' + id + '.json'
    ...
    # 3. iterate over files files and get completed status, then add transcripts to content.items.id, concatentated for ease of search
        with open(filesfilename) as files:
            ...
            for fi in filesJson:
                ...
                itemPagesObject["pages"].append(fi["id"])
                fileObj = {
                    'pageid': fi['id'],
        ...
        itemPages.append(itemPagesObject)
        # wholeItem = itemObj
...
with open('data/items.json', 'w') as dataFile:
    json.dump(itemData, dataFile)
with open('allTranscripts.json', 'w') as dataFile:
    json.dump(itemTranscriptions, dataFile)
with open('data/summary.json', 'w') as dataFile:
    json.dump(content, dataFile)
with open('data/itempages.json', 'w') as dataFile:
    json.dump(itemPages, dataFile)
